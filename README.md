...
2. Склонируйте репозиторий и перейдите в каталог:
   ```bash
   git clone https://github.com/kodjooo/raw-to-finish-data.git raw-to-finish-data-3
   cd raw-to-finish-data-3
   ```
...
5. Запустите сервис в фоне с пересборкой:
   ...
   ```
   Логи доступны через `docker compose logs -f processor`. Дополнительно можно писать в файл, указав `LOG_FILE_PATH=./logs/processor.log` (смонтировано в `docker-compose.yml`, файл появится в `./logs` на хосте). Ротация ежедневная, файлы старше 7 дней удаляются при старте. Обновление до новой версии: `git pull`, затем `docker compose build processor && docker compose up -d --force-recreate processor`.
