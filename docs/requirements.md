...
- `section_path`/`section_name`/`section_code` — в текущей реализации не используются и игнорируются; для разделов используется только `category_path`/`category`/`category_slug`.
...
- `volume`/`volume_l` и `abv`/`alcohol_percent` при записи в Bitrix всегда конвертируются в строки с точкой в качестве десятичного разделителя; для объёма добавляем суффикс ` л`, для крепости (`abv`) — пробел и знак `%` (пример: `0.75 л` и `13.5 %`), независимо от того, в каком формате их вернул LLM.
