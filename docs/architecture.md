...
3. Конфигурация и секреты
   - `.env` / `.env.example`: лежат в корне, содержат полный список переменных (см. ниже) и комментарии, где брать доступы. Эти файлы читает Docker/`pydantic-settings`.
     ...
     • `SOURCE_*`, `SINK_*` — параметры листов и колонок, в том числе `SOURCE_STATUS_IN_PROGRESS`, `SOURCE_WORKER_COLUMN`, `SOURCE_IN_PROGRESS_AT_COLUMN`, `SOURCE_IN_PROGRESS_TTL_SECONDS`, `SOURCE_CLAIM_TOKEN_COLUMN`.
