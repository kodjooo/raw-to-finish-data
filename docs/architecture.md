...
1. Общий обзор
   ...
   - Основные компоненты:
     ...
     • Source Adapter (`app/adapters/source_adapter.py`) — читает строки со статусом status_new через `GoogleSheetsClient` и `WorksheetAccessor`, помечает их как status_in_progress (если настроено) и сохраняет claim_token/worker_id, нормализует их в DTO `SourceRow`, сразу помечая пустые product_content / product_id_hash ошибками; зависшие строки в статусе «В обработке» возвращаются в «Не обработано» по TTL. Все колонки и статус «В обработке» опциональны — при пустых значениях в `.env` блок захвата/TTL отключается. В `WorksheetAccessor` добавлены ретраи для ответов Google Sheets 429/500/503 с задержками 10/30/60 секунд.
     • LLM Client (`app/services/llm_client.py`) — интеграция с OpenAI Responses API (официальный Python SDK + tenacity), формирует промпт из `product_content`, `category`, а также исходных колонок `name (en)`/`name (ru)` (с безопасными значениями по умолчанию), ориентируется на ключи ответа `category_path`/`category`/`category_slug` (а `section_*` в текущей реализации не используется), `description_html`, `volume`, `abv` и др. и правила валидного JSON (экранирование кавычек, отсутствие сырых переносов). Ответ допускает дополнительную «обёртку» (`products/items/data/product/payload/result/output`) — клиент распакует первый подходящий словарь. Поле `description` считается обязательным: если оно отсутствует или пустое, запрос повторяется и в итоге считается ошибкой. Повторяет запросы согласно `llm_max_retries`, сохраняет очищенное `llm_raw` (JSON без повторяющихся ключей, сериализованный в многострочном виде) и перед записью удаляет поле `vivino_score`, если в `product_content` нет упоминания «vivino», а также в случае невалидного JSON перепрашивает модель ещё два раза, прежде чем вернуть ошибку. Если Responses API вернул HTML-страницу ошибки 502/503, это считается ошибкой LLM и приводит к повтору запроса. При необходимости можно передать режим рассуждений через `LLM_REASONING_EFFORT`. В spirit-промпте классификатор в итоговых названиях допускается только для категорий Коньяк/Арманьяк/Бренди/Кальвадос и приводится к латинице; для рома `Гран Резерва/Gran Reserva` не вставляется в `name_main`/`name_minor` и остаётся только в `classifier`.
...
2. Поток данных
   ...
   6) Sink Adapter пишет патч в Google Sheet B:
      ...
      image_path перед записью нормализуется transform-правилом `prepend_images_path` (по умолчанию добавляется префикс `/images/`).
...
3. Конфигурация и секреты
   - `.env` / `.env.example`: лежат в корне, содержат полный список переменных (см. ниже) и комментарии, где брать доступы. Эти файлы читает Docker/`pydantic-settings`.
     ...
     • `LLM_*` — настройки OpenAI Responses API; `LLM_API_KEY` = ключ OpenAI, `LLM_MODEL` задаёт модель (например `gpt-5-nano-2025-08-07`), `LLM_REASONING_EFFORT` управляет режимом рассуждений (`none|low|medium|high`), `LLM_CATEGORY_PROFILE` выбирает категорийный промпт (`wine|sparkling|spirit`) при пустом `LLM_SYSTEM_PROMPT_PATH`, `LLM_SYSTEM_PROMPT_PATH` указывает файл системного промпта (например `config/system_prompt.txt`, `config/system_prompt_champagnes.txt` или `config/system_prompt_spirit.txt`), `LLM_USER_PROMPT_PATH` — файл пользовательского промпта. В spirit-промпте пример включает поля `aging`, `aging_barrel`, `color_shade`, `whisky_type`; в `name_main`/`name_minor` используется компонент выдержки (`aging`), а возрастные маркеры из исходных названий предварительно удаляются из блока `name_title_raw`, чтобы не дублировать срок выдержки в имени.
...
5. Логирование и мониторинг
   - Все модули пишут структурированные логи (JSON или key=value) в stdout, Docker Desktop собирает их. Дополнительно можно включить запись в файл через `LOG_FILE_PATH` (по умолчанию `./logs/processor.log`, volume смонтирован в docker-compose.yml). Логи ротируются ежедневно, файлы старше 7 дней удаляются при старте.
